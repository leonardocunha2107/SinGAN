...
def read_paint(opt):
    ...
    x = x[:,0:3,:,:]
    x=imresize_to_shape(x, [opt.nzx,opt.nzy], opt)
    return x
...
def quant(prev,device,quantk=5):
    ...
    kmeans = KMeans(n_clusters=quantk, random_state=0,n_init=3).fit(arr)
...
def quant2centers(paint, centers,quantk=5):
    ...
    kmeans = KMeans(n_clusters=quantk, init=centers,n_init=1).fit(arr)
    labels = kmeans.labels_
    #centers = kmeans.cluster_centers_
    x = centers[labels]
    x = torch.from_numpy(x)
